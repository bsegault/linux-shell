...
\begin{itemize}
    \item \texttt{+<FORMAT>} : Permet de modifier le format d'affichage de la date. \mintinline{bash}{date +%m/%d/%y} affichera la date au format JJ/MM/AAAA.
...
%Permet d'affiche le retour d'une expression arithmétique ou alphanumérique. L'ensemble des opérations simples sont disponibles. On notera tout de même la possibilité d'effectuer les opérations suivantes : 
%\begin{itemize}
...
\begin{itemize}
    ...
    \item \texttt{-n +<ligne>} : Permet d'afficher toutes les lignes à partir de \cmdref{tail} ou jusqu'à (\cmdref{head}) la ligne mentionnée.
\end{itemize}
